...
Delta Hedge Executor, and Controller Orchestration.
"""
...
class TestHydraDLMM(unittest.TestCase):
    ...
    def test_bin_shaper_curve(self):
        ...
        self.assertEqual(len(weights), 11)  # 5 below + 1 active + 5 above
        self.assertAlmostEqual(sum(weights.values()), 1.0, places=5)
        # Peak weight should be at active bin
    ...
    def test_bin_shaper_bid_ask_skew(self):
        ...
        active_bin = 1000
        # Positive momentum (uptrend) -> Ask side should have higher sum of weights
    ...
    def test_churn_gate(self):
        ...
        # Slot 1: Out of range, dwell = 1 -> should NOT rebalance (fails dwell gate)
        ...
        # Slot 2 & 3: Out of range -> dwell reaches 3 -> should rebalance if profitable
        ...
        self.assertTrue(telemetry_3["economic_hurdle_passed"])

    def test_delta_hedge_executor(self):
        hedger = DeltaHedgeExecutor(delta_threshold_pct=Decimal("0.05"))
        
        # 5 SOL spot ($700) with 0 short on $1000 capital -> Skew = 70% -> Should SELL short
        ...
        # When delta is balanced (e.g. 5 SOL spot, 4.9 SOL short) -> Skew = 1.4% < 5% -> No hedge
        order_neutral, telemetry_neutral = hedger.compute_hedge_action(
            spot_base_amount=5.0,
            spot_price_usd=140.0,
            current_perp_short_amount=4.9,
            total_strategy_capital_usd=1000.0
        )
        self.assertIsNone(order_neutral)
        self.assertEqual(telemetry_neutral["action"], "NEUTRAL")
    ...
    def test_full_controller_tick(self):
        ...
        self.assertGreater(len(result["generated_orders"]), 0)
